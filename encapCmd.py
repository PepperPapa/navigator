...
class encapCmd():
    def __init__(self):
        ...
        self.frame = []
        self.fm_start = ['68']
        self.addr = ADDRESS #通信地址后续要修改为通过数据文件读取，先临时这么用
        self.code = []
        self.data_len = []
        self.data = []
        self.crc = []
    ...
    def getFrame(self):
        ...
        '''
        #读取时间  :get time    
        if re.match(r':get time', self.cmd):
            self.code = ["11"]
            self.data_len = ["04"]
            ...
            self.send_format = self.cmd + "\t功能码:" + " ".join( self.fun_code ) 
            self.rec_format = "抄读时间为 {0[12]}{0[13]}:{0[10]}{0[11]}:{0[8]}{0[9]}"

    def sendCmd(self):
        '''发送命令到表计'''
        if self.frame:    #命令解析没有返回None表示命令解析成功
    ...
    def getResponse(self):
        self.rec_frame =['68', '02', '00', '00', '00', '00', '00', '68', '91', '07', \
                '35', '34', '33', '37', '14', '3C', '1C', 'A9', '16']  #仅用于测试
        self.rec_format = self.rec_format.format( "".join( dl645.minus33H( self.rec_frame[10:-2] ) ) ) 
